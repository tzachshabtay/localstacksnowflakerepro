...
until curl -s -d '{}' snowflake.localhost.localstack.cloud:4567/session | grep -q '{"success": true}'; do
  sleep 1
done
...
docker run \
	...
	-user=test \
	-password=test \
	-url="jdbc:snowflake://http://snowflake.localhost.localstack.cloud:4567/?db=test&schema=PUBLIC&JDBC_QUERY_RESULT_FORMAT=JSON" \
